...
abstract class QuadernoBase {
  ...
  const DEBUG_URL = '.sandbox-quadernoapp.com';
  const PRODUCTION_URL = ".quadernoapp.com";
  ...
  static function init($key, $account_id, $debug=false) {
    ...
    self::$PROTOCOL = $debug ? 'http' : 'https';
  }
 
  static function ping() {
    $url = self::$PROTOCOL . "://" . self::$ACCOUNT_ID . self::$URL . '/api/v1/ping.json';
  ...
  static function delete($model, $id) {
    $url = self::$PROTOCOL . "://" . self::$ACCOUNT_ID . self::$URL . "/api/v1/" . $model . "/" . $id . ".json";
  ...
  static function deliver($model, $id) {
    $url = self::$PROTOCOL . "://" . self::$ACCOUNT_ID . self::$URL . "/api/v1/" . $model . "/" . $id . "/deliver.json";

    return QuadernoJSON::exec($url, "GET", self::$API_KEY, "foo", null);
  }

  static function find($model, $params=null) {
    $url = self::$PROTOCOL . "://" . self::$ACCOUNT_ID . self::$URL . "/api/v1/" . $model . ".json";
    if (isset($params)) {
      $encodeQuery = '';
  ...
  static function save($model, $data, $id) {
    $url = self::$PROTOCOL . "://" . self::$ACCOUNT_ID . self::$URL . "/api/v1/" . $model;
