	...
	var that = {
	  ...
	  redraw:function(){
		...
		particleSystem.eachEdge(function(edge, pt1, pt2){
		  ...
		  ctx.strokeStyle = ( edge.data.color ) ? edge.data.color : "rgba(0,0,0, .333)";//"rgba(229,18,21, .333)"
		  ctx.lineWidth = 1
		  ctx.beginPath()
	  ...
	  initMouseHandling:function(){
		...
		var handler = {
			dblclick : function(e){
				...
				top.agd.func.open("ficha.php?m=empresa&oid=" + dragged.node.name );
			},
		  clicked:function(e){
		    var pos = $(canvas).offset();
		    _mouseP = arbor.Point(e.pageX-pos.left, e.pageY-pos.top)
		...
		// start listening
		$(canvas).mousedown(handler.clicked).dblclick(handler.dblclick);
	  },
	}
	return that;
};
