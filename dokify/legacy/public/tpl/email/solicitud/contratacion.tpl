...
<div style="padding:10px 20px 0 0">
	<img src="{$smarty.const.RESOURCES_DOMAIN}/www/images/logo.gif" style="float: right" alt="logo-dokify" />
	<h1 style="margin-top:0">{$lang.$asunto}</h1>
	<p>{$item->getUserVisibleName()|string_format:$lang.$email}</p>
	{if $estado==1}<p>{$item->obtenerUrlPreferida(true)|cat:''|string_format:$lang.puedes_comprobarlo_aqui}</p>{/if}
